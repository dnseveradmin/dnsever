...
program
    ...
    .action((env) => {
        ...
        console.log("계정이 추가되었습니다. \ndnsever show 를 해보시기 바랍니다.")
        // console.log(env.pw, env.id);
...
program
    ...
    .option("-t, --type <type>", "select record type", "ALL")
    .description(
...
program
    ...
    .option(
        "-r, --rank <path>",
        'rank used ONLY MX Record'
    )
    .option(
        "-m, --memo <path>",
        'memo used ONLY A, CNAME Record'
    )
    .requiredOption(
        "-h, --host <host>",
    ...
    .action(async (env) => {
        ...
        const getType = env.type?.trim().toUpperCase();
        const getRank = env.rank?.trim();
        const getMemo = env.memo?.trim();
...
program
    ...
    .option("-r, --rank <rank>", "MX레코드의 우선순위")
    .option("-m, --memo <string>", "Memo")
    ...
    .action(async (env) => {
        ...
        const getType = env.type?.trim().toUpperCase();
        const getRank = env.rank?.trim();
        const getMemo = env.memo?.trim().toUpperCase();
...
program
    ...
    .option("-f, --from-file <path>", "set config path", ENV_PATH)
    .requiredOption("-s, --seq <seq>", "SEQ 아이디 입력")
    .requiredOption("-t, --type <type>", "select record type")
    .description("DNSEver DNS Record 삭제\n" + "=> dnsever delete -s 46800275")
    .action(async (env) => {
        const getSeq = env.seq.trim();
        const getType = env.type.trim();
        // 설정파일이 존재하지 않을 경우 Process 종료
        const getConfig = isConfig(env);
        if (!getConfig) {
            process.exit(1);
